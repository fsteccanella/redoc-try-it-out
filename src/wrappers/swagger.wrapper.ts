...
export class SwaggerWrapper {
  ...
  private static get $authorizeModalCloseTriggers(): JQuery {
    return $(
      `${SwaggerWrapper.cfg.authorizeDoneBtnSelector}, ${SwaggerWrapper.cfg.authorizeModalCloseBtnSelector}`,
    );
  }

  private static operationSummarySelector(method: string, api: string): string {
    return SwaggerWrapper.cfg.operationSummaryPatternSelector.replace(
      /(.*)({method})(.*)({api})(.*)/,
      `$1${method}$3${api}$5`,
    );
  }
  ...
  public static hide(): void {
    ...
    SwaggerWrapper.$selectedContainerElements.removeClass(
      SwaggerWrapper.cfg.selectedOperationContainerClass,
    );
  ...
  public static selectApiSection(api: string, method: string): void {
    ...
    const $operationContainer = $apiDom.parents(
      SwaggerWrapper.cfg.operationContainerSelector,
    );
    const $operationSection = $operationContainer.parents(
      SwaggerWrapper.cfg.operationSectionContainerSelector,
    );
    ...
    $operationContainer.addClass(
      SwaggerWrapper.cfg.selectedOperationContainerClass,
    );
    $operationSection.addClass(
      SwaggerWrapper.cfg.selectedOperationContainerClass,
    );
  }
}
